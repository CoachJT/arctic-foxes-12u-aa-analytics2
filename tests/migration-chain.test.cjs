...
test('migration chain has no duplicate object names across files', () => {
  const seen = new Map();
  const collisions = [];
  for (const [file, source] of Object.entries(sources)) {
    for (const match of source.matchAll(/create (?:or replace )?(?:table|function|policy|trigger|unique index|index)\s+(?:if not exists\s+)?(?:public\.)?([a-z0-9_]+)/gi)) {
      ...
      if (seen.has(name)) collisions.push(`${name} in ${file} and ${seen.get(name)}`);
...
test('plpgsql locals do not collide with target columns in on-conflict or values clauses', () => {
  // Regression guard for the Stage 8 clean-database failures where a local
  // variable shadowed a column inside INSERT ... ON CONFLICT.
  const migration = sources['009_onboarding.sql'];
  assert.doesNotMatch(migration, /values \(organization_id, \(select auth\.uid/);
  assert.doesNotMatch(migration, /values \(team_id, \(select auth\.uid/);
  assert.doesNotMatch(migration, /values \(team_id, trim\(team_name\)/);
  assert.match(migration, /new_organization_id/);
  assert.match(migration, /new_team_id/);
  assert.match(migration, /new_season_id/);
  assert.match(migration, /new_invitation_id/);
});

test('every 2.0 table enables row level security', () => {
  const all = Object.values(sources).join('\n');
  for (const table of ['organizations', 'organization_memberships', 'seasons', 'team_branding', 'platform_roles', 'team_invitations', 'onboarding_progress', 'team_roster_players', 'team_schedule_games', 'team_games', 'team_game_player_stats', 'team_game_team_stats', 'team_season_records']) {
...
test('every security-definer RPC guards or scopes the actor and revokes public execute', () => {
  const all = Object.entries(sources)
    .filter(([file]) => ['007_platform_identity_and_roles.sql', '008_platform_admin_dashboard.sql', '009_onboarding.sql', '010_coach_qol_writes.sql'].includes(file))
    .map(([, source]) => source).join('\n');
  const functions = [...all.matchAll(/function public\.((?:admin|onboarding|coach|has_|is_)_{0,1}[a-z0-9_]*)\(/gi)]
    .map(m => m[1])
    .filter(name => !/^prevent_/.test(name) && !/^is_org_owner$/.test(name) || ['admin_list_organizations','admin_get_organization','admin_list_teams','admin_list_users','admin_get_user','admin_list_memberships','admin_list_invitations','admin_resend_invitation','admin_revoke_invitation','admin_set_beta_status','admin_list_onboarding','onboarding_ensure','onboarding_create_organization','onboarding_create_team','onboarding_create_season','onboarding_mark_step','onboarding_complete','onboarding_add_player','onboarding_invite_staff','onboarding_save_branding','coach_save_game_stats','has_platform_role','is_platform_admin','is_platform_founder','is_org_member','has_org_role','is_org_owner'].includes(name));
  ...
  const admin = sources['008_platform_admin_dashboard.sql'];
  const adminGuards = admin.match(/is_platform_admin\(\)/g) || [];
  assert.ok(adminGuards.length >= 10, 'admin RPCs must all reference is_platform_admin');
  // Onboarding mutations must all require an authenticated caller.
  const onboarding = sources['009_onboarding.sql'];
...
test('backfills are idempotent via on-conflict', () => {
  ...
  assert.match(s006, /on conflict \(slug\) do nothing/);
