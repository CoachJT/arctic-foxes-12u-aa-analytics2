...
test('all renderer scripts parse and release invariants hold',()=>{
 ...
 const pkg=require('../package.json');assert.equal(pkg.version,'3.0.16');assert.equal(pkg.build.artifactName,'Arctic-Foxes-12U-AA-Hockey-Analytics-${version}.${ext}');assert.equal(pkg.build.publish[0].repo,'arctic-foxes-12u-aa-analytics2');assert.equal(pkg.build.publish[0].owner,'CoachJT');
