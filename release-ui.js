...
function ratingConfig412(){return {...(state.ratingWeights312||A312.weights),includeIceTime:includeIceTime294()};}
function season312(games=state.savedGames||[]) {return A312.season(games,state.players,ratingConfig412());}
...
function renderEntry312() {
  ...
  const skfields=['gp','g','a','shots','plusMinus','pim','fow','fol','blocks','toiMin','tk','gv','ppg','ppp','shg','shp','gwg','gtg'];
  ...
  m.innerHTML=workspaceHeader('Enter Game Stats',`${esc(state.gameDate)} • ${esc(state.opponent)} — Save applies only to this game.`, '<button id="q295Save" class="good">Save Game Stats</button>')+
    ...
    `<div class="card team-entry314"><h2>Team stats</h2><fieldset><legend>Final score & shots</legend><div class="team-fields314">${[['Goals for','goalsFor'],['Goals against','goalsAgainst'],['Shots for','shotsFor'],['Shots against','shotsAgainst']].map(([label,k])=>`<label>${label}<input class="q295-input" type="number" min="0" step="1" data-team312="${k}" value="${A312.number(off.team[k]?.total)}"></label>`).join('')}</div></fieldset><fieldset><legend>Special teams & faceoffs</legend><div class="team-fields314">${[['PP chances','ppChances'],['PP goals','ppSuccess'],['PK chances','pkChances'],['PK kills','pkSuccess'],['Faceoff wins','fow'],['Faceoff losses','fol']].map(([label,k])=>`<label>${label}<input class="q295-input" type="number" min="0" step="1" data-team-extra312="${k}" value="${A312.number(off.team[k])}"></label>`).join('')}</div></fieldset>${[['Goals for','goalsFor'],['Goals against','goalsAgainst']].map(([label,k])=>`<fieldset><legend>${label} by period</legend><div class="team-fields314">${[['1st period','p1'],['2nd period','p2'],['3rd period','p3'],['Overtime','ot']].map(([periodLabel,period])=>`<label>${periodLabel}<input class="q295-input" type="number" min="0" step="1" data-period-group312="${k}" data-period312="${period}" value="${A312.number(off.team[k]?.[period])}"></label>`).join('')}</div></fieldset>`).join('')}</div></form>`;
...
function renderWeights312() {
  ...
  const weightLabels={production:"Goals & assists",shots:"Shots on goal",efficiency:"Ice-time efficiency",results:"Plus/minus",faceoffs:"Faceoffs",defense:"Blocked shots",discipline:"Discipline"};
  m.innerHTML=`<h2>Rating Weights</h2>${analyticsToggle294("ratingIceTime412")}<p>Off by default. When off, ice time has no effect on player ratings or MVP / stars. Recorded minutes are kept. Scoring chances do not contribute to ratings or honors.</p><p>Base weights: goals & assists 30%, shots 15%, ice-time efficiency 10%, plus/minus 15%, faceoffs 10%, blocks 15%, discipline 5%. Custom saved weights are retained. Faceoff scores are pulled toward neutral for small samples using ten neutral prior draws. Every player starts at 70.0. Game Ratings are bounded 0–100. Game and season ratings use a wider fixed scale with a neutral overall of 70. Season ratings average the underlying game scores with two neutral prior games before applying the scale, so player ordering is preserved. History shows the recalculated season rating after each game. Disabled or missing TOI and non-participating faceoff categories are omitted and remaining weights are normalized. More minutes alone never add points.</p><p>Skater impact uses goals, assists, shots, points and shots per 36 minutes (minimum 3-minute sample), plus/minus, faceoff percentage, blocks and PIM. Goalies use a separate formula: save percentage 50%, GAA 25%, shot-rate performance 10%, result 10%, shutout 5%. Minutes and shots determine confidence. Ratings are coaching indicators, not scouting grades.</p><form id="weightForm312">${Object.keys(A312.weights).map(k=>`<label style="display:inline-block;margin:8px">${weightLabels[k]||k}<input type="number" min="0" max="100" step="1" data-weight312="${k}" value="${w[k]}" style="width:65px">%</label>`).join('')}<button type="submit">Save Rating Weights</button><button type="button" id="resetWeights413">Use balanced defaults</button><span id="weightStatus312" role="status"></span></form>`;
  bindAnalyticsToggle294();
  $("#resetWeights413").onclick=()=>{delete state.ratingWeights312;save();renderWeights312();renderSeason312();$("#weightStatus312").textContent="Balanced defaults applied. Ratings recalculated.";};
...
function valueRows312(games,gameMode=false) {
  if(gameMode&&games.length===1)return A312.records(games[0]).filter(r=>r.played).map(r=>({...r,...A312.rate(r,ratingConfig412()),driver:'Game impact',keyStats:r.type==='goalie'?`${r.saves} saves • ${r.ga} GA`:`${r.g}G ${r.a}A • ${r.shots} SOG`}));
