...
function renderJerseys318(root){
 const honors=A312.honors(state.savedGames||[],state.players,ratingConfig412()),lookup=new Map(honors.totals.map(t=>[String(t.p.number),t]));
...
renderCommand313=function(){
 ...
 const honors=A312.honors(state.savedGames||[],state.players,ratingConfig412()),awards=honors.byGame.find(g=>g.id===selectedGame?.id)?.stars||[];
...
function playerHonors317(number){const t=A312.honors(state.savedGames||[],state.players,ratingConfig412()).totals.find(t=>String(t.p.number)===String(number));return t?`<section class="player-honors317"><h3>Season honors</h3>${cards31([['MVP / 1st star',t.mvp],['2nd star',t.second],['3rd star',t.third],...(t.p.pos==='G'?[['Shutouts',t.so]]:[])])}<small>MVP counts as first star · recorded goalie shutouts</small></section>`:'';}
