(function attachOrganizationContext(global) {
  function createOrganizationContext({ storage = global.sessionStorage } = {}) {
    ...
    function load(workspaces) {
      ...
      const rememberedId = storage?.getItem(
        'pucknexus-selected-organization-id'
      );

      const selected =
        context.organizations.find(item => item.id === rememberedId) ||
    ...
    function select(organizationId) {
      ...
      if (!organization) {
        throw new Error('That organization is not authorized.');
      }
    ...
    return {
      ...
      load,
      select,
      teamsForSelectedOrganization,
      clear
    };
  }

  global.FoxesOrganizationContext = {
