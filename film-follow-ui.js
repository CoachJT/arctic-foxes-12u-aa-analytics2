...
followPanel416.innerHTML='<h3>Follow players · Preview</h3><p>Attach a name to each visible player, then follow their bench crossings.</p><button id="attach416">Attach player</button><button id="startFollow416">Start following</button><button id="stopFollow416">Pause following</button><p id="followStatus416" role="status">Attach the players in a clear, paused frame.</p><div id="followList416"></div><details><summary>When a label is lost</summary><p>A briefly hidden player gets a moment to reconnect. If identity stays uncertain, playback pauses for review. Similar uniforms, overlap, and camera movement can interrupt tracking. Check recorded shifts before finalizing.</p></details>';
...
let selectedTool416=null,follow416=null,following416=false,followContext416='',followTime416=-1,followEpoch416=0;
...
function frame416(){capture416.width=Math.min(3840,v314.videoWidth);capture416.height=Math.round(capture416.width*v314.videoHeight/v314.videoWidth);captureContext416.drawImage(v314,0,0,capture416.width,capture416.height);return captureContext416.getImageData(0,0,capture416.width,capture416.height);}
function pauseFollow416(reason,invalidate=false){following416=false;followEpoch416++;v314.pause();if(invalidate)follow416?.invalidate(reason);$('#followStatus416').textContent=reason;renderFollow416();}
function renderFollow416(){const tracks=follow416?.tracks||[];$('#followList416').innerHTML=tracks.map(t=>{const p=state.players.find(p=>p.id===t.playerId);return `<div class="follow-player416"><span>#${esc(p?.number||'')} ${esc(p?.name||'')} · ${t.status==='lost'?'Reconnect':t.status==='recovering'?'Locating…':t.onIce?'On ice':'Bench'}</span><button data-reconnect416="${esc(t.playerId)}">Reconnect</button></div>`;}).join('');$('#followList416').querySelectorAll('[data-reconnect416]').forEach(b=>b.onclick=()=>openAttach416(b.dataset.reconnect416));$('#startFollow416').disabled=!tracks.length||tracks.some(t=>t.status==='lost')||following416;$('#stopFollow416').disabled=!following416;}
...
async function openAttach416(playerId=''){try{const m=media314(),z=ensure314().zones[m.clipId];if(!z||zoneDraft314||drawing314)throw Error('Save your bench line first.');if(v314.readyState<2)throw Error('Load a playable video.');pauseFollow416('Paused for player setup.');stop314();const context=JSON.stringify([state.currentGameId,m.clipId]);if(context!==followContext416){follow416=null;followContext416=context;}if(!follow416)follow416=new FoxesPlayerFollow.DetectionTracker(structuredClone(z));
 const captured={...m,gameId:state.currentGameId,frame:frame416()},epoch=followEpoch416;$('#followStatus416').textContent='Finding players in this frame…';captured.detections=await FoxesDetector.detect(captured.frame);if(epoch!==followEpoch416||captured.gameId!==state.currentGameId||captured.clipId!==currentClipId()||Math.abs(v314.currentTime-captured.videoTime)>.05)return;attachCapture416=captured;attachBox416=null;attachPoint416=null;attachFrame416.width=capture416.width;attachFrame416.height=capture416.height;attachFrame416.getContext('2d').putImageData(attachCapture416.frame,0,0);$('#attachPlayer416').innerHTML='<option value="">Choose player</option>'+options314(playerId);$('#attachPlayer416').value=playerId;$('#attachState416').value='';$('#attachStatus416').textContent='Confirming On ice starts a shift here. Confirming Bench ends any open shift here.';attachDialog416.showModal();
 }catch(e){$('#followStatus416').textContent=e.message;openTool416('follow');}}
$('#attach416').onclick=()=>openAttach416();$('#cancelAttach416').onclick=()=>attachDialog416.close();attachDialog416.addEventListener('close',()=>{if(!attachDialog416.open){attachCapture416=null;attachBox416=null;attachPoint416=null;}});
...
async function loopFollow416(){
 try{
  const context=JSON.stringify([state.currentGameId,currentClipId()]);
  if(context!==followContext416&&follow416){following416=false;followEpoch416++;follow416=null;followContext416=context;renderFollow416();}
  if(following416&&!v314.paused&&!v314.seeking&&v314.readyState>=2&&v314.currentTime-followTime416>=.12){
   if(follow416.tracks.some(t=>t.onIce!==!!state.players.find(p=>p.id===t.playerId)?.shifts?.some(s=>!s.ended)))throw Error('On-ice selections changed. Reconnect labels.');
   const tracker=follow416,epoch=followEpoch416,clipId=currentClipId(),gameId=state.currentGameId,time=v314.currentTime;
   const detections=await FoxesDetector.detect(frame416(),tracker.tracks.filter(t=>t.status==='recovering').map(t=>t.box));
   if(epoch===followEpoch416&&following416&&tracker===follow416&&clipId===currentClipId()&&gameId===state.currentGameId&&!v314.seeking){
    followTime416=time;const result=tracker.frame(detections,time);
    if(result.lost.length){const reason=tracker.tracks.find(t=>t.status==='lost')?.reason||'Reconnect labels.';pauseFollow416(reason,true);openTool416('follow');}
    else{
     $('#followStatus416').textContent=result.recovering.length?'Locating a briefly hidden player…':'Following players';
     if(result.events.length&&!commit314(g=>{for(const event of result.events){const id=crypto.randomUUID();T314.transition(g,{...event,clipId,source:'auto',detectionId:id});T314.data(g).groundTruth.push({...event,id,clipId,kind:'followed-crossing',confirmedAt:Date.now()});}})){pauseFollow416('Shift needs review. Reconnect after correcting it.',true);openTool416('review');}
    }
   }
  }
  paintFollow416();
 }catch(e){pauseFollow416(e.message,true);openTool416('follow');}
 requestAnimationFrame(loopFollow416);
}
