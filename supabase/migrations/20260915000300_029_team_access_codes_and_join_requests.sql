-- Draft-only: Team Access Codes and Join Requests.
-- Additive: no existing membership, invite, onboarding, or platform-role data changes.
...
create unique index team_access_codes_one_live_per_team_idx
  on public.team_access_codes(team_id) where revoked_at is null;
...
create unique index team_join_requests_one_pending_per_user_team_idx
...
-- Codes are never readable through the data API. All access happens through guarded RPCs.
revoke all on public.team_access_codes, public.team_join_requests from anon;
revoke all on public.team_access_codes, public.team_join_requests from authenticated;
...
create or replace function public.create_or_regenerate_team_access_code(
  target_team_id uuid, plaintext_code text, expires_at_input timestamptz default null
) returns table (code_id uuid, expires_at timestamptz)
language plpgsql security definer set search_path = public as $$
declare normalized_code text; new_id uuid;
begin
  ...
  update public.team_access_codes set revoked_at = now(), regenerated_at = now()
    where team_id = target_team_id and revoked_at is null;
  insert into public.team_access_codes(team_id, code_hash, expires_at, created_by)
    values (target_team_id, crypt(normalized_code, gen_salt('bf')), expires_at_input, (select auth.uid())) returning id into new_id;
...
language plpgsql security definer set search_path = public as $$
declare code public.team_access_codes%rowtype; result public.team_join_requests%rowtype; normalized_code text;
begin
  if (select auth.uid()) is null then raise exception 'Sign in is required to request team access.'; end if;
  normalized_code := upper(regexp_replace(coalesce(access_code_input,''), '[^A-Za-z0-9]', '', 'g'));
  select * into code from public.team_access_codes
    where revoked_at is null and (expires_at is null or expires_at > now()) and code_hash = crypt(normalized_code, code_hash)
    limit 1;
  ...
  if exists (select 1 from public.team_memberships where team_id = code.team_id and user_id = (select auth.uid()) and status = 'active') then raise exception 'You already have access to this team.'; end if;
  insert into public.team_join_requests(team_id, access_code_id, requester_id, requested_role_id)
    values (code.team_id, code.id, (select auth.uid()), requested_role_input)
...
begin
  ...
  if approve then
    insert into public.team_memberships(team_id, user_id, role_id, status, invited_by)
      values (request_row.team_id, request_row.requester_id, request_row.requested_role_id, 'active', (select auth.uid()))
      on conflict (team_id, user_id) do update set role_id = excluded.role_id, status = 'active', invited_by = excluded.invited_by;
  end if;
  return request_row;
end; $$;
